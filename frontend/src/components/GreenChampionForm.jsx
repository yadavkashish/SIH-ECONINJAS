...
export default function GreenChampionForm() {
  ...
  const [alreadyApplied, setAlreadyApplied] = useState(false); // NEW: tracks if user applied

  // ✅ Initialize form & check application
  useEffect(() => {
  async function init() {
    try {
      const application = await checkApplicationStatus();

      if (application && application._id) {
        setMessage(`✅ You have already applied. Status: ${application.status}`);
        setAlreadyApplied(true);
        return; // Don't fetch wards if already applied
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      address: {
        house: form.houseNo.trim(),
        locality: form.locality.trim(),
        city: form.city,
        pin: form.pin.trim(),
        selectedWard: form.selectedWard ? Number(form.selectedWard) : null,
      },
      ngoName: form.ngoIsPart ? form.ngoName.trim() : "",
    ...
    try {
      ...
      if (resp && resp.id) {
        ...
        setAlreadyApplied(true); // hide form after submission
  ...
  return (
    <div style={container}>
      <h1 style={{ textAlign: "center", marginBottom: 24, color: "#2c7a7b" }}>
        🌱 Green Champion Application
      </h1>
      ...
      {/* ONLY show form if user hasn't applied */}
      {!alreadyApplied && (
        <form
        ...
        >
          {/* Personal Info */}
          <div style={card}>
            <h2 style={{ marginBottom: 16, color: "#2b6cb0" }}>
          ...
          <div style={card}>
            ...
            <div style={fieldGroup}>
              ...
              <div style={field}>
                ...
                <input
                  style={{ ...input, fontWeight: "bold", background: "#f0f8ff" }}
                  readOnly
                  value={suggestedWard || ""}
                />
              </div>
          ...
          <div style={card}>
            <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input
                ...
                onChange={handleChange}
              />
              I agree to volunteer as a Green Champion and follow municipal
              guidelines.
            </label>
          </div>
